...
import { ALL_MONTH_HEADERS, getHeaderDateRange, TimeInterval } from "@/utils/dateUtils";
...
export function MonthRangePicker({ date, onDateChange, className, minDate, maxDate }: MonthRangePickerProps) {
  ...
  const [pendingRange, setPendingRange] = useState<DateRange | undefined>(date);

  const yearsInHeaders = useMemo(() => {
    const years = new Set<number>();
    ALL_MONTH_HEADERS.forEach(header => {
      const d = getHeaderDateRange(header, "Month" as TimeInterval)?.startDate;
  ...
  const handleFromSelect = (selectedMonth: Date) => {
    ...
    let to = pendingRange?.to ? startOfMonth(pendingRange.to) : startOfMonth(selectedMonth);
    if (isBefore(to, from)) {
  ...
  const handleToSelect = (selectedMonth: Date) => {
    ...
    const from = pendingRange?.from ? startOfMonth(pendingRange.from) : startOfMonth(selectedMonth);
    if (isBefore(to, from)) {
      setPendingRange({ from: startOfMonth(selectedMonth), to });
    } else {
      setPendingRange({ from, to });
    }
  };
  ...
  const formatButtonLabel = () => {
    ...
    const toMonthStr = date?.to ? formatDateFn(date.to, "MMM yyyy") : "";
    return `${fromMonthStr}${toMonthStr ? ` - ${toMonthStr}` : ""}`;
  };
          ...
          <div className="flex flex-col gap-4 p-4">
            <div className="flex flex-col gap-2">
              <div className="text-center font-medium">
                {date?.from ? formatDateFn(date.from, "MMM yyyy") : "No from date"} -
                {date?.to ? formatDateFn(date.to, "MMM yyyy") : "No to date"}
              </div>
              ...
              <div className="flex gap-6 overflow-x-auto">
                ...
                <div className="grid grid-cols-3 gap-2 w-full">
                  {Array.from({ length: 12 }).map((_, i) => {
                    ...
                    const isSelected = pendingRange?.from && isSameMonth(month, pendingRange.from);
                    const isCurrentFrom = date?.from && isSameMonth(month, date.from);
                    return (
                      <Button
                        key={`from-${i}`}
                        ...
                        className={cn(
                          "h-16 relative",
                          isCurrentFrom && "ring-2 ring-blue-500"
                        )}
                        onClick={() => handleFromSelect(month)}
                      >
                        {formatDateFn(month, 'MMM yyyy')}
                        {isCurrentFrom && <span className="absolute bottom-1 text-xs">(Selected)</span>}
                      </Button>
                    );
                  })}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 w-full">
                  {Array.from({ length: 12 }).map((_, i) => {
                    ...
                    const isSelected = pendingRange?.to && isSameMonth(month, pendingRange.to);
                    const isCurrentTo = date?.to && isSameMonth(month, date.to);
                    const isInRange = pendingRange?.from && pendingRange?.to &&
                                      isAfter(month, pendingRange.from) && isBefore(month, pendingRange.to);
                    return (
                      <Button
                        key={`to-${i}`}
                        variant={isSelected ? "default" : "outline"}
                        className={cn(
                          ...
                          isCurrentTo && "ring-2 ring-blue-500"
                        )}
                        onClick={() => handleToSelect(month)}
                      >
                        {formatDateFn(month, 'MMM yyyy')}
                        {isCurrentTo && <span className="absolute bottom-1 text-xs">(Selected)</span>}
                      </Button>
                    );
                  })}
                </div>
