...
// Generate daily headers for monthly planning (30 days)
export const ALL_DAILY_HEADERS = Array.from({ length: 30 }, (_, i) => {
  const date = new Date(2024, 0, i + 1); // January 2024, days 1-30
...
const generateLobInputs = (periods: string[], isMonthly: boolean = false) => {
  ...
  periods.forEach(period => {
    // For Monthly Plan: Generate weekly data (same as tactical plan)
    // When "Month" interval is selected, it will show monthly aggregated data
    if (isMonthly) {
      // Monthly data should be higher as it represents a full month
      volume[period] = Math.floor(Math.random() * 40000) + 20000; // Monthly volume
      aht[period] = Math.floor(Math.random() * 10) + 5;
    } else {
      // Weekly data (same as tactical plan)
      volume[period] = Math.floor(Math.random() * 10000) + 2000;
      aht[period] = Math.floor(Math.random() * 10) + 5;
    }
  });
  return { volume, aht };
    ...
    <TooltipProvider>
      <header className="sticky top-0 z-50 bg-background p-4 border-b border-border rounded-tl-lg rounded-tr-lg">
        <div className="flex flex-col md:flex-row justify-between mt-[-15px] ml-[-5px] md:items-center gap-4">
          <h1 className="text-xl font-semibold text-foreground">Monthly Planning</h1>
...
export default function CapacityInsightsPageV2({ navigateSimulator, businessId }) { // Renamed component
  ...
  const [selectedPlanTimeInterval, setSelectedPlanTimeInterval] = useState<TimeInterval>("Month");
  ...
  const [selectedChartTimeInterval, setSelectedChartTimeInterval] = useState<TimeInterval>("Month");
          ...
          let lobTotalBaseRequiredMinutesForCalcContext: number | null = null;
            ...
            if (currentSelectedTimeInterval === "Month") {
              ...
              if (weeklyInputs.length > 0) {
                ...
                const avgMetricsKeys: (keyof TeamPeriodicMetrics)[] = ["aht", "shrinkagePercentage", "occupancyPercentage", "backlogPercentage"];
